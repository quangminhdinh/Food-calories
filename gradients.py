...
def squared(width, ratio, startcolor, endcolor):
    """
    Draws a linear sqared gradient on a square sized surface and returns
    that surface.
    """
    bigSurf = pygame.Surface((width*ratio, width)).convert_alpha()
    bigSurf.fill((0,0,0,0))
    dd = -1.0/(width/2)
    sr, sg, sb, sa = endcolor
    er, eg, eb, ea = startcolor
    ...
    for currentw in range(int(width/2), 0, -1):
        ...
        if abc <= 20:
            break
        if pos <= int(width / abc):
            draw_rect(bigSurf, (er + int(rm*currentw),
                                eg + int(gm*currentw),
                                eb + int(bm*currentw),
                                ea + int(am*currentw / bcd)), pygame.Rect(pos*ratio, pos, 2*currentw*ratio, 2*currentw ))
        else:
            bcd += 0.05
            abc -= 1
            draw_rect(bigSurf, (er + int(rm * currentw),
                                eg + int(gm * currentw),
                                eb + int(bm * currentw),
                                ea + int(am * currentw / bcd)),
                      pygame.Rect(pos * ratio, pos, 2 * currentw * ratio, 2 * currentw))
    return bigSurf
...
def squared_func(width, startcolor, ratio, endcolor, Rfunc = (lambda x:x), Gfunc = (lambda x:x), Bfunc = (lambda x:x), Afunc = (lambda x:1), offset=(0,0)):
    """
    Draws a linear sqared gradient on a square sized surface and returns
    that surface.
    """
...
##        pos = (width/2)-currentw
        rect = pygame.Rect(0, 0, 2*currentw*ratio, 2*currentw )
        rect.center = ((width/2+offset[0]) * ratio, width/2+offset[1])
...
def draw_squared(surface, startpoint, endpoint, ratio, startcolor, endcolor, Rfunc = (lambda x:x), Gfunc = (lambda x:x), Bfunc = (lambda x:x), Afunc = (lambda x:1), mode=0):
    """
    Instead of returning an Surface, this function draw it directy onto the 
    given Surface and returns the rect.
    """
    ...
    bigSurf = squared_func(width, startcolor, ratio, endcolor, Rfunc, Gfunc, Bfunc, Afunc)
