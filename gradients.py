...
def radial_func_offset(radius, startcolor, endcolor, Rfunc=(lambda x: x), Gfunc=(lambda x: x), Bfunc=(lambda x: x),
    ...
    """
    bigSurf = pygame.Surface((2 * radius, 2 * radius))
...
def squared_func(width, startcolor, ratio, endcolor, Rfunc=(lambda x: x), Gfunc=(lambda x: x), Bfunc=(lambda x: x),
...
def draw_squared(surface, startpoint, endpoint, ratio, startcolor, endcolor, Rfunc=(lambda x: x), Gfunc=(lambda x: x),
                 Bfunc=(lambda x: x), Afunc=(lambda x: 1), mode=0):
    """
    Instead of returning an Surface, this function draw it directy onto the 
    given Surface and returns the rect.
...
def genericFxyGradient(surf, clip, color1, color2, func, intx, yint, zint=None):
    ...
    surff = pygame.Surface((w, h)).convert_alpha()
    # generate values
    for i in range(h):
        for j in range(w):
